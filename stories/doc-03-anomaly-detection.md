...
Для примера выберем запись-событие с максимальным NLL:
...
Тут, как интересно, оказались сразу два маркера аномальности, и максимальная оценка модели и одновременно новое событие, что сразу маркирует такой кластер как аномальный. Однако не все так хорошо, как хотелось бы. Например, есть образец совершенно неподходящий под определение аномального:

```bash
❯ cat 207920-26884038-logs_scored.jsonl | jq 'select(."nll" == 21.010391235351562)'
{
  "pipeline_id": "207920",
  "build_id": "26884038",
  "line_no": 14,
  "raw_line": "Pipeline variable 'FullKitPath' set to: /home/vsts-01/_work/26/a/bundler-kit.tar.gz",
  "section_name": "Get Kit Artifact#2",
  "cluster_id": 26666,
  "event_id": "E26666",
  "position": 1631,
  "nll": 21.010391235351562,
  "is_anomaly": true
}
```

Как оценить такое соседство? Получается поиск аномалий с помощью обученной LogBERT не дал абсолютно правильного результата? Вот как можно примириться с данным фактом:

- Никто и никогда не обещал 100% успешности такого решения. Для этого и делается специальная оценка успеха с помощью метрик. И это надо будет сделать в итоге.

- LogBERT надо понимать как способ очистить логи от малорелевантных по отношению к ошибке строк, а не буквально указать на причину таковой. Этот вопрос и должна решить LLM, обрабатывающая инференс от LogBERT&

- Альтернативой выбранному подходу является просто направить все строки лога, все 2 тычячи, в промпте LLM и так поискать в них смысл и объяснение ошибки. Очевидный вопрос: в каком случае успех анализа проблемы с помощью LLM будет выше, на сырых исходных логах, или на тех, что были предварительно очищены и максимально наполнены аномальными строками с помощью LogBERT?

Ответы более чем очевидны. Таким образом, найденный "happy path" должен с минимальными потерями сохраниться в процессе квантизации модели!
